...
ws.export_vtu(f'outputs/vtuFiles/{ws.name}.vtu')
...
ws.compute_centerline_orientations()
print(f'Direction map: {ws.direction_map}')

ws.export_vtu(f'outputs/vtuFiles/{ws.name}_withOrientation.vtu')
...
# mesh = ws.apply_smoothing({'laplacian':2})
...
# ws.invert_faces()
# ws.export_stl(f'outputs/stlFiles/{ws.name}_inverted.stl')
