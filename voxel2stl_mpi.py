...
print(f"MPI initialized: Rank {rank} of {size}", flush=True)

# Force all ranks to sync here before continuing
#comm.Barrier()
...
from hermes.mpi import process_single_volume_task, run_mpi_cli



def voxel2stl(croppingFlag, cropSettings, surfaceSettings, savingOptions):
    
    # Only rank 0 handles file setup
    if rank == 0:
    ...
    # Broadcast savingOptions to all ranks (in case path was modified)
    savingOptions = comm.bcast(savingOptions, root=0)
    ...
    # Build list of all tasks on rank 0, then scatter
    all_tasks = []
    ...
    if rank == 0:
        ...
        for surf in filenames:
            ...
            if croppingFlag == 'Regular':
                if volumeLength == 0:
                    ...
                    all_tasks.append({
                        'surf': surf,
                        'filevoxel': filevoxels[tempNameIndex],
                    ...
                    for i in range(numVolumes):
                        ...
                        all_tasks.append({
                            ...
                            'temp_number': temp_number,
                            'volumeLength': volumeLength,
                            'corner': None,  # Will be generated with seed
                            'voxelsLength': voxelsLength,
                            'seed': seed
                        })
                        temp_number += 1
        ...
        print(f"Total tasks to distribute: {len(all_tasks)}")
    
    # Broadcast total task count
    all_tasks = comm.bcast(all_tasks, root=0)
    
    # Distribute tasks among ranks
    local_tasks = np.array_split(all_tasks, size)[rank]
    
    print(f"Rank {rank}: Processing {len(local_tasks)} tasks")
    
    # Process local tasks
    local_results = []
    for task in local_tasks:
        result = process_single_volume_mpi(task, surfaceSettings, savingOptions)
    ...
    if rank == 0:
        ...
        print(f"Completed {len(results)} volumes")
        return results
    
    return None

def process_single_volume_mpi(task, surfaceSettings, savingOptions):
    """
    Worker function to process a single volume with MPI.
    """
    surf = task['surf']
    filevoxel = task['filevoxel']
    temp_number = task['temp_number']
    volumeLength = task['volumeLength']
    voxelsLength = task['voxelsLength']
    
    if task['corner'] is None:
        # Generate random corner using seed
        random.seed(task['seed'])
        corner = np.zeros(3, dtype='int')
        corner[0] = random.randint(0, int(voxelsLength[0] - volumeLength[0] / filevoxel))
        corner[1] = random.randint(0, int(voxelsLength[1] - volumeLength[1] / filevoxel))
        corner[2] = random.randint(0, int(voxelsLength[2] - volumeLength[2] / filevoxel))
    else:
        corner = np.array(task['corner'], dtype='int')
...
def getstl(surfacename, tifvoxelsize, temp_number,volumeLength, corner, surfaceSettings, savingOptions): 
    ...
    # Surface file name
    tempName = surfacename[:-4]+'_V%i_%i-%i-%i-%i-%i-%i'%(temp_number,corner[0],corner[1],corner[2],volumeLength[0],volumeLength[1],volumeLength[2])
    
    if volumeLength != 'Full':
        # adjust nummber volumelength 
        volumeLength[0] = int(volumeLength[0]/tifvoxelsize)
        volumeLength[1] = int(volumeLength[1]/tifvoxelsize)
        volumeLength[2] = int(volumeLength[2]/tifvoxelsize)
        
        # Crop the volume of interest
        temp_volume = image_volume[corner[0]:corner[0]+volumeLength[0],corner[1]:corner[1]+volumeLength[1],corner[2]:corner[2]+volumeLength[2]]
    ...
    if not volume_check:
        ...
        tempName, vertices, faces = fixMesh_trimesh(tempName, vertices, faces)
...
def stlSmoothing(FileName, vertices, faces, surfaceSettings ):
    ...
    if surfaceSettings['RemoveIslandsFlag']: 
        removeIsland = '_NI'
        vertices,faces = remove_floating_islands_Stl_trimesh(vertices, faces)
    else:
        removeIsland = ''
...
def remove_floating_islands_Stl_trimesh(vertices, faces):
    ...
    largest = max(components, key=lambda m: m.faces.shape[0])
...
def fixMesh_trimesh(FileName, vertices, faces):
    ...
    print(f"{FileName} fixed! Watertight: {mesh.is_watertight}")
    
    return FileName, mesh.vertices, mesh.faces
...
def run_voxel2stl():
    ...
    filenames = [r'D1_0.9988.tif']#,'D1_0.9988.tif','D2_0.9988.tif','D3_0.9988.tif','D4_0.9988.tif','D5_0.9988.tif','D6_0.9988.tif','D7_0.9988.tif','D8_0.9988.tif','D9_0.9988.tif'] # one or more Ex: ['file1.tif', 'file2.dat', ...]
    
    filevoxels = [0.9988]#,0.9988,0.9988,0.9988,0.9988,0.9988,0.9988,0.9988,0.9988,0.9988] # one or more correspondig to filenames Ex: [1, 1.8, ...]
    ...
    if croppingFlag == 'Regular':
        ...
        volumeLength = [300,300,300] # In um or enter 0 for Full volume
        numVolumes = 3 # Number of volumes or enter 0 for Lego
...
def run_cli(argv=None):
    return run_mpi_cli(argv, comm=comm)


def _process_cli_task(task):
    return process_single_volume_task(task)
