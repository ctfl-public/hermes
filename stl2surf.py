...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 22 17:24:31 2021
...
# error message
# TO RUN: python stl2surf.py Targetfile.stl outputfile.stl '10**-6'


def error(str=None):
  if str: print("ERROR:",str)
  else: print("Syntax: stl2surf.py stlfile surffile convertion(optional)")
  sys.exit()
...
import sys,re
if (len(sys.argv) != 3 and len(sys.argv) != 4): error()
...
if len(sys.argv) == 3:
    ...
    Convertion = float(eval(sys.argv[3]))
...
if name:
  ...
  print("# SPARTA surface file, from STL file %s\n"%(sys.argv[1]), file=fp)
...
for i,vert in enumerate(verts):
  print(i+1,(float(vert[0])*Convertion),(float(vert[1])*Convertion),(float(vert[2])*Convertion), file=fp)
